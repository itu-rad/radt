#!/usr/bin/env python3
"""ImageNet Training Script (from: https://raw.githubusercontent.com/huggingface/pytorch-image-models/refs/heads/main/train.py)
...
def main():
    ...
    with radt.run.RADTBenchmark() as run:
        ...
        global_epoch = 0
        mlflow = run
        mlflow.log_metric("ML - Epoch", 0, 0)
        results = []
        try:
            for epoch in range(start_epoch, num_epochs):
                ...
                mlflow.log_metric("ML - Epoch", epoch + 1, epoch + 1)
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if not args.prefetcher:
            input, target = input.to(device=device, dtype=model_dtype), target.to(
                device=device
            )
        ...
        if update_idx % args.log_interval == 0:
            ...
            if utils.is_primary(args):
                ...
                ml_metrics["ML - Iterations/s"] = (
                    update_sample_count / update_time_m.avg
                )
                ml_metrics["ML - Loss"] = losses_m.avg
                ml_metrics["ML - Data Time"] = data_time_m.avg
                ml_metrics["ML - Update Time"] = update_time_m.avg
                ml_metrics["ML - Update Speed"] = (
                    update_sample_count / update_time_m.avg
                )
                ml_metrics["ML - Learning Rate"] = lr
...
def validate(
    ...
    ml_metrics = {}
    ml_metrics["ML - Val Loss"] = losses_m.avg
    ml_metrics["ML - Val Top1"] = top1_m.avg
    ml_metrics["ML - Val Top5"] = top5_m.avg
    mlflow.log_metrics(ml_metrics, global_epoch)
